...
REGISTER_OP("ThreeInterpolateGpu")
    .Input("points: float32")
    .Input("idx: int32")
    .Input("weight: float32")
    .Output("out: float32")
...
class ThreeInterpolateGPUOp: public OpKernel{
    public:
        ...
        void Compute(OpKernelContext * context) override {
            ...
            auto out_flat = out_tensor->flat<float>();
            float *out = &(out_flat(0));
            threenninterpolateGpuLauncher(b,m,c,n,points,idx,weight,out);
        }
};
REGISTER_KERNEL_BUILDER(Name("ThreeInterpolateGpu").Device(DEVICE_GPU),ThreeInterpolateGPUOp);


REGISTER_OP("ThreeInterpolateGpuGrad")
    .Input("points: float32")
    .Input("idx: int32")
    .Input("weight: float32")
    .Input("grad_out: float32")
...
class ThreeInterpolateGpuGradOp: public OpKernel{
    public:
        explicit ThreeInterpolateGpuGradOp(OpKernelConstruction * context):OpKernel(context){}
        ...
        void Compute(OpKernelContext * context) override {
            ...
            threenninterpolateGpuGradLauncher(b,n,c,m,grad_out,idx,weight,grad_points);
        }
};
REGISTER_KERNEL_BUILDER(Name("ThreeInterpolateGpuGrad").Device(DEVICE_GPU),ThreeInterpolateGpuGradOp);